    ...
    public abstract class AbstractRestClient {
        ...
        public HttpResponse PostFiles(string path, UploadFile[] uploadFiles, ParameterMap parameters) {
            ...
            try {
                ...
                // upload the files
                var resp = HttpUploadHelper.Upload(urlConnection, uploadFiles, form);
                using (resp) {
        ...
        public HttpResponse Delete(string path, string accept, ParameterMap parameters) { return Execute(new HttpDelete(path, parameters) {Accept = accept}); }
        ...
        public void BasicAuth(string username, string password) {
            string encoded = $"Basic {Convert.ToBase64String(Encoding.UTF8.GetBytes($"{username}:{password}"))}";
